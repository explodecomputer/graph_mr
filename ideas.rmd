...
lpos$what <- rep(c("pos", "neg"), times=nsim)
lpos$model <- "Model1"
...
l2path$what <- rep(c("pos", "neg"), times=nsim)
l2path$model <- "Model2"
...
l3path$what <- rep(c("pos", "neg"), times=nsim)
l3path$model <- "Model3"
...
A <- bind_rows(list(lpos, l2path, l3path, lneg))
...
To generate the FDR, a different dataset was generated with no simulated causal paths. A graph traversal algorithm is applied to find a path of significant links to make a chain from 1 to 6. This is not constrained to go through 2,3,4,5, it can go through any path to get from 1 to 6. 
...
AAg <- AA %>% group_by(what, model, strategy) %>% summarise(propsig = sum(pval < 0.01) / n())

ggplot(AAg, aes(x=as.factor(model), y=propsig)) +
...
$$
FDR(i,j) = \alpha + \sum^n_{k=2} \alpha^k \frac{(n - 2)!}{(n - k)!}
$$

This has the following relationship with growing graph sizes
...
calc_number_of_paths <- function(n, thresh, maxpathlength = n)
{
	...
	vec[1:min(2, maxpathlength)] <- 1
	# fac <- factorial(n - 2)
	for(i in 3:maxpathlength)
	{
		# vec[i] <- fac / factorial(n - i)
		# print(fac / factorial(n - i))
		vec[i] <- prod(n - c(2:(i-1)))
		# print(prod(n - c(2:(i-1))))
	}


	p <- thresh ^ c(1:maxpathlength)
	dat <- data.frame(k = 1:maxpathlength, npath = vec, p = p, fdr = vec * p)
	return(dat)
}

l <- list()
param <- expand.grid(n = 3:300, thresh=c(0.05, 0.04, 0.03, 0.02, 0.01))
for(i in 1:nrow(param))
{
	l[[i]] <- calc_number_of_paths(param$n[i], param$thresh[i], min(param$n[i]))
	l[[i]]$n <- param$n[i]
...
What does this look like e.g. for 50 trait graph with $\alpha = 0.05$?
...
We could limit the maximum path length to e.g. six. This would basically 




- Does not account for sparse graphs that have known number of significant edges

- Limit the possible chain length
