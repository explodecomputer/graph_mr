...
The false discovery rate is estimated as follows. For a graph where a path of 1 causing 6 is simulated, paths are searched for that link **6 causing 1**, i.e. the reverse, for which there should be no paths.
...
this can be simplified to be computationally tractable for large graphs and has the following relationship with growing graph sizes
...
- Does not account for sparse graphs that have known number of significant edges - that would make it graph specific.
- Limit the possible chain length for a significant
